...
def text_generation(rnn_type='basic_rnn', input_file="shakespeare.txt", sample_len=2000, seed="KING LEAR"):
    if rnn_type == "basic_rnn":
        # specify the sequence length, hidden size and number of layers
        seq_len, hidden_size, num_layers = 100, 128, 2
        # specify train opts
        train_opts = {
            "num_epochs": 120,
            "lr": 0.001,
        ...
        # specify the sequence length, hidden size and number of layers
        seq_len, hidden_size, num_layers = 100, 256, 2
        # specify train_opts
        train_opts = {
            "num_epochs": 120,
            "lr": 0.001,
