...
class EarlyStopping:
    ...
    def early_stopper(self, validation_acc, epoch):
...
def train(model, train_ds, val_ds, train_opts, exp_dir=None):
    ...
    for epoch in range(num_epochs):
        ...
        if early_stopping.early_stopper(val_acc, epoch):
            print('Early Stopper engaged. Highest validation accuracy achieved at epoch '
                  f'{early_stopping.best_epoch + 1} with accuracy of {early_stopping.max_validation_acc:.2%}')
            break
    ...
    # load best model
    model.load_state_dict(load(path.join(exp_dir, f"checkpoint_{early_stopping.best_epoch + 1}.pt")))
