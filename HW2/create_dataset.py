...
def create_dataset(data_path, output_path=None, contrast_normalization=False, whiten=False):
    ...
    data_tr, data_te, sets_tr, label_tr, class_names = data_dict.get('data_tr'), data_dict.get('data_te'), \
                                                       data_dict.get('sets_tr'), data_dict.get('label_tr'), \
    ...
    if data_path == "image_categorization_dataset.pt":
        ...
        mean_tr = torch.mean(data_tr, dim=0)
        mean_te = torch.mean(data_te, dim=0)

        data_tr = torch.sub(data_tr, mean_tr)
        data_te = torch.sub(data_te, mean_te)
